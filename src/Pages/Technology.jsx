...
export const Technology = () => {
  ...
  useEffect(()=>{
    fetch('https://node-backend-project-1wnx3825i-ramanakorada2611.vercel.app/technologyLatestPosts').then(response=>response.json())
    .then(json=>setPost(json))
  },[])
  useEffect(()=>{
    fetch('https://node-backend-project-1wnx3825i-ramanakorada2611.vercel.app/technologyArticles').then(response=>response.json())
    .then(json=>setPost1(json))
  },[])
  useEffect(()=>{
    fetch('https://node-backend-project-1wnx3825i-ramanakorada2611.vercel.app/technologyPosts').then(response=>response.json())
    .then(json=>setPost2(json))
  },[])
   useEffect(()=>{
    fetch('https://node-backend-project-1wnx3825i-ramanakorada2611.vercel.app/technologystories').then(response=>response.json())
 ...
 <div className='modified-container'>
{
  post1 && post1.map((item)=>{
    return(
      <Cards1 name={item.name} image={item.image} descripiton={item.descripiton} date={item.date}/>
    )
  })
} </div>
